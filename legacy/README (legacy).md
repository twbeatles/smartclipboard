...
- 5분 자동 잠금 타이머
- 손상된 설정은 잠금 화면 `Reset 보관함`으로 초기화 복구

### ⚡ 클립보드 액션 자동화
- 패턴 매칭 기반 자동 처리
- **v10.2**: URL 제목 가져오기 개선 (타임아웃/에러 처리 강화)
- `fetch_title`은 로컬/사설/메타데이터 주소를 기본 차단
- 전화번호 자동 포맷팅 (`02`, 일반 지역번호, `0505`, `15xx/16xx/18xx`)
- 이메일 정규화
- 텍스트 변환 (대소문자, 트림 등)
- **v10.2**: 정규식 패턴 유효성 검증
...
- JSON 마이그레이션 모드 (히스토리 메타데이터 + 컬렉션 메타데이터 포함, 스니펫/규칙/핫키/보안 보관함 제외)
- `FILE` 항목은 복원 전에 누락 경로(stale) 여부를 미리 표시
- 백업 및 마이그레이션 용이
...
- `fetch_title`은 후행 문장부호를 제거한 첫 URL만 사용하고, 로컬/사설/메타데이터 주소 및 비 HTML 응답을 차단
- 전화번호 자동 포맷 범위를 `02`, 일반 지역번호, `0505`, `15xx/16xx/18xx` 대표번호까지 확장
- 보안 보관함은 salt/verification을 함께 검증하고, 손상 상태를 `Reset 보관함` 경로로 복구
- `custom_replace`는 빈 문자열 치환을 허용해 삭제 치환에 사용할 수 있음
- `FILE` 항목은 목록/상세/미니 창에서 stale/missing 경로 수를 먼저 보여주며 부분 복원 정책은 유지
- JSON 마이그레이션 문구는 실제 범위(히스토리 메타데이터 + 컬렉션) 기준으로 정리
