...
- **통합 디자인 토큰 시스템**: 모든 테마가 동일한 radius/간격/elevation/포커스 언어를 공유 (`features/settings/styles/tokens.py`)
- 명확한 버튼 위계: 그라데이션 주요 버튼 · 고스트 도구 칩 · 상단 빠른 실행 칩 · 위험(삭제) 아웃라인
- 은은한 선택/호버 하이라이트(브랜드 틴트 + 좌측 액센트)와 일관된 포커스 링
- **v10.1**: 향상된 마이크로 인터랙션 (호버/포커스 효과)
- 온보딩 가이드가 포함된 빈 상태 UI
- 제목·상세 위계를 갖춘 플로팅 토스트 알림 (슬라이드 애니메이션 + 컬러 액센트)
- 테마 연동 프라이버시/일시정지 상태 칩
- 미니 창 모드 지원
