...
fn test_collection_crud_and_cascade_null() {
    ...
    // 1. Add Collection
    let col_id = db
        .add_collection("테스트 프로젝트", "🧪", "#ec4899")
        .expect("add collection");
    let (item_id, _) = db
        .add_item("컬렉션 연동 아이템", None, "TEXT")
        .expect("add item");
    ...
    // 2. Update Collection
    db.update_collection(col_id, "수정된 프로젝트", "🔬", "#3b82f6")
    ...
    assert_eq!(
        detail_after.collection_id, None,
        "collection_id must be set to NULL on collection delete"
    );
}
...
fn test_snippet_crud() {
    ...
    let snip_id = db
        .add_snippet("환영 메시지", "환영합니다!", Some("Alt+H"), "인사")
        .expect("add snippet");
    let snippets = db.get_snippets().expect("get snippets");
    assert!(snippets
        .iter()
        .any(|s| s.id == snip_id && s.name == "환영 메시지"));

    db.update_snippet(
        snip_id,
        "환영 메시지 v2",
        "스마트클립보드에 오신 것을 환영합니다!",
        None,
        "인사",
    )
    .expect("update");
    let snippets_updated = db.get_snippets().expect("get snippets");
    let snip = snippets_updated
        .iter()
        .find(|s| s.id == snip_id)
        .expect("found");
...
fn test_action_palette_execution() {
    ...
    assert_eq!(
        execute_action("uppercase", "hello world").unwrap(),
        "HELLO WORLD"
    );
    assert_eq!(
        execute_action("lowercase", "HELLO WORLD").unwrap(),
        "hello world"
    );
