...
class LegacyLoaderTests(unittest.TestCase):
    ...
    def test_payload_failure_falls_back_to_src(self):
        ...
        self.assertTrue(
            "payload hash mismatch" in reason
            or "payload size mismatch" in reason
            or "marshal" in reason
            or "typeerror" in reason
        )
    ...
    def test_payload_hash_mismatch_falls_back_to_src(self):
        manifest_path = Path("smartclipboard_app/legacy_main_payload.manifest.json")
        original_manifest = manifest_path.read_text(encoding="utf-8")
        try:
            manifest = json.loads(original_manifest)
